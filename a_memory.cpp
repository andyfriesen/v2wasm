...
void *valloc(int amount, char *desc, int owner)
{
	...
	V_memset((char *) chunks[numchunks].pointer +
...
int vfree(void *ptr)
{
   ...
   if (i == -1)
   {
      Log(va("vfree: Attempted to free ptr 0x%08X that was not allocated. [dumping mem report]", ptr));
      MemReport();
      return -1;
   }
   FreeChunk(i);
...
void MemReport(void)
{
   ...
   Log("========================================");
   Log(va("Chunks currently allocated: %d (MAXCHUNKS %d)", numchunks, MAXCHUNKS));
   Log(va("%d total bytes allocated. ", TotalBytesAllocated()));
#ifdef PARANOID
   Log(va("PARANOID is ON. (pad size: %d pad value: %d)", PADSIZE, PADFILLVALUE));
#else
   Log("PARANOID is OFF.");
#endif
   Log("");
   ...
   for (i=0; i<numchunks; i++)
   {
       Log(va("[%3d] Ptr at: 0x%08X size: %8d owner: %3d desc: %s",
