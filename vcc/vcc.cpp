...
#define BUILD_TAG "clang from the future!"
...
void vprint(const char* message, ...) {
    va_list lst;
    char string[1024];
...
void vcc_compile_mode_map(char* filename) {
    ...
    *x = 0;
    ...
    current_func =
        0; // DO *NOT* FORGET THIS; lest CheckLibFunc giveth you a man-beating.
...
void DumpSystemVCS() {
    ...
    // these two still use separate counters because arrayed vars increase the
    // total overall count
...
void vcc_compile_mode_all() {
    ...
    while (auto ent = readdir(dir)) {
        ...
        if (path.size() > 4 && path.substr(path.size() - 4) == ".map") {
            vcc_compile_mode_map(const_cast<char*>(path.c_str()));
        }
    }
