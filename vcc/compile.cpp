...
void HandleFunction()
{
	...
	switch (tok.index)
	{
		...
		case 95: vc_fwopen(); break;
                ...
                case 99: GenericFunc(100, 5); break;  // silhouette
...
void ProcessFor()
{
	unsigned char *src1, *src2, *loopstartpos, *srctmp;
	...
	loopstartpos=(unsigned char *) (int) code - (int) outbuf;
...
void SecondPass()
{
	...
	dprint("system.vc: %i ints, %i strings, %i functions",
		varlist.number_nodes(), strlist.number_nodes(), functionlist.number_nodes());
}
