...
static void Conlib_ConsoleBG()
{
	...
	// if we couldn't find it, notify user
		if (!pic)
		{
			Console_Printf(va("%s: error opening console graphic", Con_GetArg(1).c_str()));
			Console_Printf("");
			return;
		}
	...
	// we found it, but it's not the correct size
		...
		{
			vfree(pic);
			Console_Printf(va("%s: invalid console graphic", Con_GetArg(1).c_str()));
			Console_Printf("");
			return;
		}

	// anything else is success; free old, set loaded as new
		if (consolebg) // first time it'll be nothing; don't make mem routines dump a report
			vfree(consolebg);
		consolebg	=pic;
	}
	break;
...
static void Conlib_ListMounts()
{
	...
	case 1: {
		...
		{
			Console_Printf(va("File %s contains %d files.", pack[n].mountname, pack[n].numfiles));
		}
		Console_Printf("");
	}
	break;
 ...
 switch (Con_NumArgs())
  {
   case 1: do
    ...
    default: Conlib_ListCmds_Syntax();
             break;
	}
}
...
static void Conlib_Vid_Mode()
{
	...
	{
	case 3:
		if (gfx.SetMode( Con_GetArg(1).toint(),
                         Con_GetArg(2).toint(),
                         gfx.bpp*8,
                         gfx.IsFullScreen() ))
		{
...
static void Conlib_Map()
{
	...
	case 1:
		...
		//sprintf(strbuf,"MAP stats for Ä%s~ - Å%d~ layers",mapname,numlayers); Console_Printf(strbuf);
		Console_Printf(va("MAP stats for %s - %d layers", mapname, numlayers));

		//sprintf(strbuf,"Base dimensions Å%d~ x Å%d~", layer[0].sizex, layer[0].sizey);
		//Console_Printf(strbuf);
		Console_Printf(va("Base dimensions %dx%d", layer[0].sizex, layer[0].sizey));
		...
		//sprintf(strbuf,"Å%d~ active zones.",numzones); Console_Printf(strbuf);
		...
		while (key_last()!=SCAN_Q)
		{
			...
			for(y=1; y<ty-2; y++)
			{
				for(x=1; x<tx-3; x++)
				{
					...
					if (n<numtiles)
					{
						if (!a) CopySpriteClip(x*16,y*16,16,16,(unsigned char*)((unsigned int)vsp+((y-1)*(tx-4)+x-1+k)*256));
						else CopySpriteClip(x*16,y*16,16,16,(unsigned char*)((unsigned int)vsp+tileidx[(y-1)*(tx-4)+x-1+k]*256));
					}
				}
...
static void Conlib_Warp()
{
	...
	case 3:
		...
		if (warpx<0 || warpx>=layer[0].sizex || warpy<0 || warpy>=layer[0].sizey)
		{
			Console_Printf(va("Coordinates out of bounds (%d,%d)", layer[0].sizex, layer[0].sizey));
			return;
		}
...
static void Conlib_Entity()
{
	...
	case 2: {
		...
		//sprintf(strbuf,"tx: Ç%d~ ty: Ç%d~", entity[i].tx, entity[i].ty); Console_Printf(strbuf);
		Console_Printf(va("tx: %d ty: %d", entity[n].tx, entity[n].ty));
		//sprintf(strbuf,"CHR index: %d", entity[i].chrindex); Console_Printf(strbuf);
...
static void Conlib_CurPos()
{
	...
	{
	case 1:
		//sprintf(strbuf,"xwc: Ç%d~ ywc: Ç%d~", player->x, player->y); Console_Printf(strbuf);
		Console_Printf(va("xwc: %d ywc: %d", player->x, player->y));
		//sprintf(strbuf,"xtc: Ç%d~ ytc: Ç%d~", player->x>>4, player->y>>4); Console_Printf(strbuf);
...
static void Conlib_SpawnEntity()
{
	...
	case 4: {
		int n=AllocateEntity(
			//atoi((char *) args[1]), atoi((char *) args[2]), (char *) args[3]);
