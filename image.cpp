...
static byte* Image_LoadBMPBuf(const char* fname)
{
	...
	for (n=0; n<256; n++)
	{
		gfx.gamepal[n*3 + 0]	= (byte) (bmppalette[n].rgbBlue  >> 2);
		gfx.gamepal[n*3 + 1]	= (byte) (bmppalette[n].rgbGreen >> 2);
		gfx.gamepal[n*3 + 2]	= (byte) (bmppalette[n].rgbRed   >> 2);
	}
...
static byte* Image_LoadPNGBuf(const char* fname)
{
 ...
 buf=new byte[image_width*image_length*3];           // we'll return a 24 bit image, and let the other image stuff convert it to 16bpp
...
// if in hicolor mode, must convert 8-bit to 16-bit
	...
	// convert to 16-bit
		...
		{
/*			if (!ptr[n])
				hip[n] = gfx.trans_mask; //(16 == hicolor) ? 0xF81F : 0x7C1F;
			else*/
