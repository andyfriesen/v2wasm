                    ...
                    if (b)
                     {
                      ...
                      b=(word) (b&31);
                      // Gah! @_@
                      ((word *)vsp)[n]=(r<<11)+(g<<6)+b; // v4 vsps are 1:5:5:5, 16bit color is 5:6:5
                     }
                    else
                     ((word *)vsp)[n]=gfx.trans_mask;
                   }
                  break;
          case 5: vread(&bufsize,4,f);
...
// adjust data for currene pixel depth
 if (ver<4)
  {
   // nothing needs to happen to an 8bit VSP if we're in 8bit mode, so leave it alone.
   if (gfx.bpp==2)
    {
     c=(word*)valloc(16*16*numtiles*2,"LoadVSP: Tile Data (hicolour)",OID_IMAGE);
 ...
 else
  {
   ...
   // TODO: allow hicolour VSPs in 8bit mode? (do some closest-matching-colour type stuff)
   for (n=0; n<16*16*numtiles; n++)
...
void FreeVSP()
{
 ...
 if (flipped!=NULL)
  {
   vfree(flipped);   flipped=NULL;
  }
 if (tileidx!=NULL)
  {
   vfree(tileidx);   tileidx=NULL;
  }
 if (vspmask!=NULL)
  {
   vfree(vspmask);   vspmask=NULL;
  }
 if (vsp!=NULL)
  {
   vfree(vsp);       vsp=NULL;
...
void WriteBMP24(void) // the Speed Bump
{
  ...
  i=(gfx.scrx*gfx.scry*3)+54;       fwrite(&i,1,4,bmpf); // size of file=size of screen + size of header
  ...
  i=gfx.scrx;           fwrite(&i,1,4,bmpf); // bmp width
  i=gfx.scry;           fwrite(&i,1,4,bmpf); // bmp height
  w=1;                  fwrite(&w,1,2,bmpf); // just one color plane
  w=24;                 fwrite(&w,1,2,bmpf); // 24 bit image
  i=0;                  fwrite(&i,1,4,bmpf); // no compression
  i=gfx.scrx*gfx.scry*3;            fwrite(&i,1,4,bmpf);
...
// Write PCX header
 ...
 w1=(unsigned short)(gfx.scrx-1); fwrite(&w1, 1, 2, pcxf);  // xmax = 319;
 w1=(unsigned short)(gfx.scry-1); fwrite(&w1, 1, 2, pcxf);  // ymax = 199;
 w1=(unsigned short)gfx.scrx; fwrite(&w1, 1, 2, pcxf);  // hres = 320;
 w1=(unsigned short)gfx.scry; fwrite(&w1, 1, 2, pcxf);  // vres = 200;
 ...
 w1=(unsigned short)gfx.scrx; fwrite(&w1, 1, 2, pcxf); // number of bytes per line
...
// FIXME: it doesn't work. :P (switching to windowed mode results in a window,
//but no change in display mode, switching to fullscreen results in a crash)
