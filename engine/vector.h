...
class vector_t {
   protected:
    int F_hunksize, F_size, F_capacity;
    T *F_data;

    int getcapacity() const { return F_capacity; }
    ...
    int setsize(int size) {
        ...
        if (newcapacity >
            F_capacity)  // if the new capacity is larger than the current
        {
   ...
   public:
    ...
    vector_t<T>() : F_hunksize(256), F_capacity(0), F_data(NULL), F_size(0) {}
    vector_t<T>(const vector_t<T> &rhs)
        : F_hunksize(rhs.F_hunksize), F_capacity(0), F_data(NULL), F_size(0) {
        push_vector(rhs);
    }
    ~vector_t<T>() { free(); }
    ...
    T &push_vector(const vector_t<T> &v) {
        ...
        for (int n = 0; n < v.size(); n++) base[n] = v[n];
        return *base;
    }
    ...
    T &push(T item) { return push() = item; }
    T &insert(int index, T item) {
        push();
        for (int n = size() - 1; n > index; --n) F_data[n] = F_data[n - 1];
    ...
    T &pop() { return F_data[--F_size]; }

    int gethunksize() const { return F_hunksize; }
...
template <class T>
class queue_t : public vector_t<T> {
   protected:
    int F_head, F_tail;
