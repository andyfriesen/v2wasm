...
/*
    ...
    *   Obligatory tweaking to better suit the new graphics driver interface.
    ...
    8       January         2000
    <aen>
    *       Revamp/tinkering. Interface is: Image_LoadBuf(), Image_Width(),
   Image_Length()
...
static byte *Image_LoadBMPBuf(const char *fname) {
    ...
    if (!(bmpfile = vopen(fname)))
        Sys_Error("Could not open BMP file %s.", fname);
    ...
    if (colordepth == 8) {
        ...
        if (gfx.bpp == 1) {
            vclose(bmpfile);
            Sys_Error(
                va("Attempted to load truecolour BMP %s in 256 colour mode.  "
                   "Aborting",
                    fname));
        }
...
static gif_header *get_gif_header(FILE *fp) {
    ...
    h->screendeep = read_word_lbf(fp);
    image_length = h->screendeep;
...
static u16 word_mask_table[] = {0x0000, 0x0001, 0x0003, 0x0007, 0x000F, 0x001F,
...
byte *Image_LoadBuf(const char *filename) {
    ...
    case 0:
        ptr = Image_LoadPCXBuf(filename);
        break;
    case 1:
        ptr = Image_LoadGIFBuf(filename);
        break;
    case 2:
        ptr = Image_LoadBMPBuf(filename);
        break;
    case 3:
        ptr = Image_LoadPNGBuf(filename);
        break;
