...
void vc_ScaleSprite()
{
    ...
    if (!lucentmode)
     gfx.ScaleSprite(x,y,source_width,source_length,dest_width,dest_length,(byte*)ptr);
    else
     gfx.ScaleSpriteLucent(x,y,source_width,source_length,dest_width,dest_length,(byte*)ptr,lucentmode);
}
...
void vc_PartyMove()
{
	...
	while (entity[playernum].movecode)
	{
        ...
        /* To prevent crashing, we have to update the controls, and check to see if alt-x is pressed.
           Otherwise, if the user accidently directs the player into an obstruction, the player is
...
void vc_WrapBlit()
{
    ...
    if (!lucentmode)
     ...
     gfx.WrapBlitLucent(offsetx,offsety,width,length,(byte*)ptr,lucentmode);
}
...
void vc_TWrapBlit()
{
    ...
    if (!lucentmode)
     ...
     gfx.TWrapBlitLucent(offsetx,offsety,width,length,(byte*)ptr,lucentmode);
}
...
void vc_RotScale()
{
    ...
    if (!lucentmode)
     gfx.RotScale(x,y,width,length,(float) (angle*3.14159/180.0),(float)(scale/1000.0) ,(byte*)ptr);
    else
     gfx.RotScaleLucent(x,y,width,length,(float) (angle*3.14159/180.0),(float)(scale/1000.0) ,(byte*)ptr,lucentmode);
}
...
// This is the location of the first tile we draw, it'll be at the left hand edge of the screen
	...
	x_sub = -(x_map & 15);                    // get subtile position while we still have pixel precision
	y_sub = (y_map & 15);
    ...
    do
     {
      if (*source)
       gfx.CopySprite(x,y,16,1,(byte*)(vsp) + (16*16*tileidx[*source] + (16*y_sub))*gfx.bpp);
...
// This is the location of the first tile we draw, it'll be at the left hand edge of the screen
    ...
    do
     {
      if (*source)
       gfx.TCopySprite(x,y,16,1,(byte*)(vsp) + (16*16*tileidx[*source] + (16*y_sub))*gfx.bpp);
...
void vc_TScaleSprite()
{
    ...
    if (lucentmode)
     gfx.TScaleSpriteLucent(x, y, source_width, source_length, dest_width, dest_length, (byte *)ptr,
      lucentmode);
    else
     gfx.TScaleSprite(x, y, source_width, source_length, dest_width, dest_length, (byte *)ptr);
}
...
//	vcreturn=(int)fopen((const char*)filename,"wb");
	...
	Log(va(" --> VC opened %s for writing, handle %i.", filename.c_str(), vcreturn));
}
