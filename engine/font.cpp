...
void font_t::paintchar(const char ch) {
    ...
    int offset = (F_selected * 96 * F_cellwidth * F_cellheight) +
                 ((ch - 32) * F_cellwidth * F_cellheight);
    // move there
...
int font_t::loadheader(VFILE* vfp) {
    ...
    if (ver != 1) {
        vclose(vfp);
        Sys_Error(
            "font_t::loadheader: %s: incorrect version number (reported %d).\n",
            getfilename(), ver);
    }
...
int font_t::loadfromfile(const char* filename) {
    ...
    if (!f) {
        ...
        // get cumulative size of all subsets in this font & resize data to
        // match
...
int Font_GetWidth(int slot) { return fontcontroller.getfont(slot).getwidth(); }
