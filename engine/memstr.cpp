...
int memorystream_t::setsize(unsigned long size) {
    ...
    if (newcapacity > F_capacity) {
        ...
        if (!setcapacity(newcapacity)) return 0;
    }
    // reset size
    F_size = size;
...
long memorystream_t::read(void *dest, long length) {
    ...
    if (F_position < 0) return -3;
    // suffix out of bounds
    if (F_position >= F_size) return -5;
    // suffix clip
    if (F_position + length >= F_size) length = F_size - F_position;
    // perform read
...
long memorystream_t::write(const void *source, long length) {
    ...
    if (F_position + length > F_size) {
        ...
        if (!setsize(F_position + length)) return -4;
    }
    // perform write
...
/*void memorystream_t::readstring(string_t& s, long length)
{
        s.setsize(length + 1);
...
long memorystream_t::insert(long position, const void *source, long length) {
    ...
    // expand buffer
    if (!setsize(F_size + length)) return -4;
...
long memorystream_t::remove(long position, long length) {
    ...
    // suffix clip
    if (position + length >= F_size) length = F_size - position;
...
long memorystream_t::read(memorystream_t &dest, long length) {
    ...
    if (dest.F_position < 0) return -3;
    // dest suffix out of bounds
    if (dest.F_position >= dest.F_size) return -5;
    // dest suffix clip
...
long memorystream_t::write(const memorystream_t &source, long length) {
    ...
    if (source.F_position < 0) return -3;
    // source suffix out of bounds
    if (source.F_position >= source.F_size) return -5;
    // source suffix clip
