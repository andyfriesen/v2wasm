               ...
               if (c)
                {
                 if (masked)
                  {
                   if (color_mapped)
                    gfx.TCopySpriteLucent(y_sub,y,16,16,vsp+(c<<8)*gfx.bpp,color_mapped);
                   else
                    gfx.TCopySprite(y_sub,y,16,16,vsp+(c<<8)*gfx.bpp);
                  }
                 else
                  {
                   if (color_mapped)
                    gfx.CopySpriteLucent(y_sub,y,16,16,vsp+(c<<8)*gfx.bpp,color_mapped);
                   else
 ...
 while (rpos < rstring.length())
  {
   switch (rstring[rpos])
    {
     case '1': BlitLayer(0); curlayer++; break;
     case '2': BlitLayer(1); curlayer++; break;
     case '3': BlitLayer(2); curlayer++; break;
     case '4': BlitLayer(3); curlayer++; break;
     case '5': BlitLayer(4); curlayer++; break;
     case '6': BlitLayer(5); curlayer++; break;
     case 'e':
