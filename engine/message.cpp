	...
	: public linked_node
{
	int compare(void* c)
	{
		return ((message_t *)c)->expire_at > expire_at;
	}

	message_t(string_k s, int expire)
	: text(s), expire_at(expire) {}
...
void Message_Send(string_k text, int duration)
{
	...
	messages.insert_tail((linked_node *)new message_t(text, systemtime+duration));
