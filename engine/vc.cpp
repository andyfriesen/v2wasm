...
int ReadInt(char category, int loc, int ofs)
{
	...
	case op_HVAR1:
		...
		*/
		...
		case 29:return zones[ofs].script;					// Zara made me add it!
		case 30:return zones[ofs].percent;
		default:Sys_Error("ReadInt: funky offset to var1's: %d",loc);
		}
...
void WriteInt(char category, int loc, int ofs, int value)
{
	...
	case op_HVAR1:
		...
		case 29:if (ofs<0 || ofs>=numzones)
				{
					if (v2_touchy)
						Sys_Error("WriteInt: bad offset to zone.event: %d (%d total)",
							ofs,numzones);
				}
			    zones[ofs].script=value;					// Zara made me add it!
				return;
		
		case 30:if (ofs<0 || ofs>=numzones)
					if (v2_touchy)
						Sys_Error("WriteInt: bad offset to zone.chance: %d (%d total)",
							ofs,numzones);
